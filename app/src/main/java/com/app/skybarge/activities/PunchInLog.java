 ...
 */
public class PunchInLog extends AppCompatActivity implements OnCustomItemClicListener, ApiResponse {

    Context context;
    RecyclerView mRecyclerView;
    ModelPunchIn itemList;
    AdapterPunchInList adapterPunchInList;
    ArrayList<ModelPunchIn> arrayList;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_punchinloglist);
    ...
    public void PunchLogRefresh() {
        ...
        if (AppUtils.isNetworkAvailable(context)) {
            ...
            String url = JsonApiHelper.BASEURL + JsonApiHelper.PUNCHIN_LOG + "/" + AppUtils.getUserId(context);
        ...
        try {
            if (method == 1) {
                if (response.getString("status").equalsIgnoreCase("1")) {
                    ...
                    JSONArray array = response.getJSONArray("Attendance");
                    arrayList.clear();
                    for (int i = 0; i < array.length(); i++) {

                        JSONObject jo = array.getJSONObject(i);
                        itemList = new ModelPunchIn();

                        itemList.setPunchIn_id(jo.getString("punchIn_id"));
                        itemList.setPunch_in_time(jo.getString("punch_in_time"));
                        itemList.setPunch_out_time(jo.getString("punch_out_time"));
                        itemList.setPunch_date(jo.getString("punch_date"));
                        itemList.setRowType(1);

                        arrayList.add(itemList);
                    }
                    adapterPunchInList = new AdapterPunchInList(context, this, arrayList);
                    mRecyclerView.setAdapter(adapterPunchInList);
